...
def login(event):         #defining function Login
    if tx1.get()==""or tx2.get()=="":
        ...
        def add_criminal(event):
            ...
            labl3.grid(row=6, column=1, padx=20, pady=10)
            criminal_cnic = Entry(f4, width=80,bd='5')
            criminal_cnic.grid(row=6, column=2, padx=10, pady=10)
            labl4 = Label(f4, text="CRIMINAL STATUS",fg="white",bg="#333333", font={"Cooper Black"})
            labl4.grid(row=8, column=1, padx=20, pady=10)
            c2 = StringVar()
            criminal_status = Entry(f4,bd='5', width=80, textvariable=c2)
            criminal_status.grid(row=8, column=2, padx=20, pady=10)
        ...
        def view_criminal(event):
            ...
            def Search2(event):
                ...
                j = list(sum(m, ()))
                if val4 in j:
                    c.execute("SELECT *FROM add_criminal_1 where CRIMINAL_CNIC=?", (val4,))
                    rec = c.fetchone()
                    conn.commit()
                    conn.close()
                    messagebox.showinfo('Successfull !!',
